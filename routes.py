...
def stats(fileid):
    ...
    for f in filenames_and_ids:
        ...
        if int(fileid) == f[0]:
            ...
            return render_template("stats.html", players=result, players_columns=players_columns, log_link=users.log_link(), 
...
def liked():
    ...
    fileid = request.form["fileid"]
    if users.user_id() == 0:
        return render_template("error.html", error="You you need to be logged in to leave a like!")
